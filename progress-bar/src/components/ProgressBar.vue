  ...
  <div style="display: flex">
    <div class="progress-bar-wrapper">
      <div class="progress-bar">
        <div ref="percentageRef" class="percentage"></div>
      </div>
      <div class="divide-count" v-if="props.divide">
        <div v-for="index in getDivideCount" :key="index">
          {{ (props.maxProgress / props.divideCount) * index }}
        </div>
        <div>{{ props.maxProgress }}</div>
      </div>
    </div>
    <div class="cur-progress" v-if="props.numToRight">{{ curProgress }}%</div>
  </div>
</template>
<script setup>
